...
(defun laooree-modeline--major-mode-name ()
  ...
  (propertize (format " %s" (replace-regexp-in-string "-" " " (replace-regexp-in-string "-mode\\'" "" (symbol-name major-mode)))) 'face `(:foreground ,(face-attribute 'font-lock-comment-face :foreground)))
  )
...
(defvar-local laooree-modeline-major-mode
    '(:eval (laooree-modeline--major-mode-name))
...
;; Turn off automatic indentation in text-mode.
