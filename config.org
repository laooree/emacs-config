...
(defface laooree-modeline-white-standout
  '((t (:foreground  "#000000" :background "#ffffff")))
  "Face for elements that should stand out in the mode-line, with white background."
  :group 'laooree-modeline-faces)

(defface laooree-modeline-magenta-standout
  '((t (:foreground  "#000000" :background "#ff66ff")))
  "Face for elements that should stand out in the mode-line, with magenta background."
  :group 'laooree-modeline-faces)
#+end_src
*** Functions
...
(defun laooree-modeline--evil-state-tag-propertized ()
  "Get evil-mode state tag with propertized faces."
  (cond ((string= (laooree-modeline--evil-state-tag) "(N)") (laooree-modeline--evil-state-tag))
        (t (propertize (format "%s" (laooree-modeline--evil-state-tag)) 'face 'laooree-modeline-white-standout))))
...
(defun laooree-modeline--defining-kbd-macro ()
  ...
  (propertize (format "%s" (cond (defining-kbd-macro "   Rec ")
                                 (t "")))
              'face 'laooree-modeline-magenta-standout))
...
(defvar-local laooree-modeline-evil-state-tag-propertized
  '(:eval
    (when (mode-line-window-selected-p)
      (laooree-modeline--evil-state-tag-propertized)))
  "Expression to show current evil state on the focused mode-line.")
