...
Working with bleeding edge packages sometimes requires to update the built-in ones (to avoid building emacs from master branch).
...
Shows current/total lines, or percentage when `display-line-numbers-mode' is active."
  (let* ((indicator
    ...
    (laooree-modeline--with-indicator
     (concat indicator (when (< (window-body-width) (window-total-width)) " "))
