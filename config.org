...
(set-face-attribute 'mode-line-highlight  nil :box nil :underline nil)
#+end_src
...
#+begin_src emacs-lisp :tangle "init.el"
;; Default fonts
(let ((mono-font-name (cond
                       ((eq system-type 'windows-nt) "Iosevka NFM")
                       ((eq system-type 'gnu/linux)  "Iosevka Nerd Font Mono")))
      (prop-font-name "Arimo Nerd Font")
      (font-size (cond
...
*** Faces
#+begin_src emacs-lisp :tangle "init.el"
(defgroup laooree-modeline nil
  "My custom mode-line."
  :group 'mode-line)

(defgroup laooree-modeline-faces nil
  "My custom mode-line faces."
  :group 'laooree-modeline)

(defface laooree-modeline-idle-indicator
  '((t (:foreground  "#ffffff" :background "#303030")))
  "Face for elements inside an idle indicator."
  :group 'laooree-modeline-faces)

(defface laooree-modeline-idle-delimiter
  '((t (:foreground  "#303030" :background "#555555")))
  "Face for delimiters of an idle indicator."
  :group 'laooree-modeline-faces)

(defface laooree-modeline-subtle-indicator
  '((t (:foreground  "#989898" :background "#303030")))
  "Face for elements inside an subtle indicator."
  :group 'laooree-modeline-faces)

(defface laooree-modeline-subtle-delimiter
  '((t (:foreground  "#303030" :background "#555555")))
  "Face for delimiters of an subtle indicator."
  :group 'laooree-modeline-faces)

(defface laooree-modeline-teal-indicator
  '((t (:foreground  "#000000" :background "#00d3d0")))
  "Face for elements inside a teal indicator."
  :group 'laooree-modeline-faces)

(defface laooree-modeline-teal-delimiter
  '((t (:foreground  "#00d3d0" :background "#555555")))
  "Face for delimiters of a teal indicator."
  :group 'laooree-modeline-faces)

(defface laooree-modeline-orange-indicator
  '((t (:foreground  "#000000" :background "#fec43f")))
  "Face for elements inside a orange indicator."
  :group 'laooree-modeline-faces)

(defface laooree-modeline-orange-delimiter
  '((t (:foreground  "#fec43f" :background "#555555")))
  "Face for delimiters of a orange indicator."
  :group 'laooree-modeline-faces)

(defface laooree-modeline-yellow-indicator
  '((t (:foreground  "#ffffff" :background "#7a6100")))
  "Face for elements inside a yellow indicator."
  :group 'laooree-modeline-faces)

(defface laooree-modeline-yellow-delimiter
  '((t (:foreground  "#7a6100" :background "#555555")))
  "Face for delimiters of a yellow indicator."
  :group 'laooree-modeline-faces)

(defface laooree-modeline-cyan-indicator
  '((t (:foreground  "#ffffff" :background "#004065")))
  "Face for elements inside a cyan indicator."
  :group 'laooree-modeline-faces)

(defface laooree-modeline-cyan-delimiter
  '((t (:foreground  "#004065" :background "#555555")))
  "Face for delimiters of a cyan indicator."
  :group 'laooree-modeline-faces)

(defface laooree-modeline-purple-indicator
  '((t (:foreground  "#ffffff" :background "#2266ae")))
  "Face for elements inside a purple indicator."
  :group 'laooree-modeline-faces)

(defface laooree-modeline-purple-delimiter
  '((t (:foreground  "#2266ae" :background "#555555")))
  "Face for delimiters of a purple indicator."
  :group 'laooree-modeline-faces)

(defface laooree-modeline-magenta-indicator
  '((t (:foreground  "#000000" :background "#ff66ff")))
  "Face for elements inside a magenta indicator."
  :group 'laooree-modeline-faces)

(defface laooree-modeline-magenta-delimiter
  '((t (:foreground  "#ff66ff" :background "#555555")))
  "Face for delimiters of a teal indicator."
  :group 'laooree-modeline-faces)
#+end_src
*** Functions
...
#+begin_src  emacs-lisp :tangle "init.el"
(defun laooree-modeline--with-subtle-indicator (instring)
  "Returns a propertized subtle indicator containing the input string."
  (concat (propertize "" 'face 'laooree-modeline-subtle-delimiter)
          (propertize instring 'face 'laooree-modeline-subtle-indicator)
          (propertize "" 'face 'laooree-modeline-subtle-delimiter)
          ))

(defun laooree-modeline--with-indicator (instring indicator-color foreground-color)
  "Returns the input string inserted in a rounded indicator, applying foreground-color if non-nil."
  (let* ((processed-string (copy-sequence instring))
         (face-spec (if foreground-color
                        `(:background ,indicator-color :foreground ,foreground-color)
                      `(:background ,indicator-color))))
    (add-face-text-property 0 (length processed-string)
                            face-spec
                            nil processed-string)
    (concat (propertize "" 'face `(:background "#555555" :foreground ,indicator-color))
            processed-string
            (propertize "" 'face `(:background "#555555" :foreground ,indicator-color)))))

(defun laooree-modeline--buffer ()
  "Returns the buffer name, using propertization to show its status."
  (if (eq buffer-file-name nil)
      (laooree-modeline--with-indicator (propertize (buffer-name) 'face 'bold) "#303030" nil)
    (cond ((eq buffer-read-only t) (laooree-modeline--with-indicator (propertize (concat " " (buffer-name)) 'face 'bold) "#004065" nil))
          ((buffer-modified-p) (laooree-modeline--with-indicator (propertize (concat "󰙏 " (buffer-name)) 'face 'bold) "#00d3d0" "#000000"))
          (t (laooree-modeline--with-indicator (propertize (buffer-name) 'face 'bold) "#004065" nil))
          )))

(defun laooree-modeline--evil-state-tag ()
  "Get evil-mode state tag without delimiters."
  (replace-regexp-in-string " <\\([^>]*\\)> " "\\1" evil-mode-line-tag))

(defun laooree-modeline--evil-state-indicator ()
  "Get a propertized indicator with evil-mode state tag."
  (cond ((string= (laooree-modeline--evil-state-tag) "N") (laooree-modeline--with-indicator (laooree-modeline--evil-state-tag) "#303030" nil))
        ((string= (laooree-modeline--evil-state-tag) "O") (laooree-modeline--with-indicator (laooree-modeline--evil-state-tag) "#fec43f" "#000000"))
        (t (laooree-modeline--with-indicator (laooree-modeline--evil-state-tag) "#2266ae" nil))
        ))
...
(defun laooree-modeline--flymake-diagnostics ()
  ...
  (concat (propertize (format " %s " (laooree-modeline--flymake-count :error)) 'face 'error)
          (propertize (format " %s " (laooree-modeline--flymake-count :warning)) 'face 'warning)
          (propertize (format "󱂻 %s"   (laooree-modeline--flymake-count :note)) 'face 'success)))
...
(defun laooree-modeline--defining-kbd-macro ()
  ...
  (cond (defining-kbd-macro (laooree-modeline--with-indicator "  Rec" "#ff66ff" "#000000"))
        (t "")))
#+end_src
...
#+begin_src emacs-lisp :tangle "init.el"
(defvar-local laooree-modeline-buffer
  '(:eval (laooree-modeline--buffer))
  "Expression to show current buffer status and name on the mode-line.")

(defvar-local laooree-modeline-evil-state-indicator
  '(:eval
    (when (mode-line-window-selected-p)
      (laooree-modeline--evil-state-indicator)))
  "Expression to show current evil state on the focused mode-line, inside a rounded indicator.")

(defvar-local laooree-modeline-flymake-diagnostics
  '(:eval (when (and flymake-mode (mode-line-window-selected-p))
            (laooree-modeline--with-unpropertized-indicator (laooree-modeline--flymake-diagnostics) "#303030")))
  "Expression to show flymake diagnostics count on the focused mode-line.")

(defvar-local laooree-modeline-major-mode
  '(:eval (laooree-modeline--with-subtle-indicator (laooree-modeline--major-mode-name)))
...
(defvar-local laooree-modeline-keyboard-macro-indicator
  '(:eval (laooree-modeline--defining-kbd-macro))
...
#+begin_src emacs-lisp :tangle "init.el"
(dolist (expression '(laooree-modeline-buffer
...
** copilot.el
#+begin_src emacs-lisp :tangle "init.el"
;; copilot.el requires very recent track-changes
