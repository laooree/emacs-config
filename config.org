...
(use-package modus-themes
  ...
  :hook (after-init . (lambda ()
                        (set-face-attribute 'mode-line nil :box '(:line-width 2 :color "#959595") :underline nil)
                        (set-face-attribute 'mode-line-active    nil :box '(:line-width 2 :color "#959595") :underline nil)
                        (set-face-attribute 'mode-line-inactive  nil :box '(:line-width 2 :color "#2d2d2d") :underline nil)
                        (set-face-attribute 'mode-line-highlight nil :box '(:line-width 2 :color "#959595") :underline nil)))
  :config
  (modus-themes-load-theme 'modus-vivendi)
  (unless (display-graphic-p) ;; override tty cursor color
    (send-string-to-terminal (format "\e]12;%s\a" (modus-themes-get-color-value 'cursor))))
  )
...
#+begin_src  emacs-lisp :tangle "init.el"
(defun laooree-modeline--with-indicator (instring fg-color bg-color text-color)
  "Returns `instring' inside an indicator with specified `fg-color' and `bg-color'.
If non-nil, `text-color' specifies instring color."
  (concat (propertize "" 'face `(:foreground ,fg-color :background ,bg-color))
          (let ((str (copy-sequence instring)))
            (if text-color
                (add-face-text-property 0 (length str) `(:background ,fg-color :foreground ,text-color) t str)
              (add-face-text-property 0 (length str) `(:background ,fg-color) t str))
            str)
          (propertize "" 'face `(:foreground ,fg-color :background ,bg-color))))
...
(defun laooree-modeline--evil-state-indicator ()
  ...
  (cond ((string= (laooree-modeline--evil-state-tag) "N")
         (laooree-modeline--with-indicator (laooree-modeline--evil-state-tag)
                                           (modus-themes-get-color-value 'bg-dim)
                                           (modus-themes-get-color-value 'bg-mode-line-active)
                                           nil))
        (t (laooree-modeline--with-indicator (laooree-modeline--evil-state-tag)
...
(defun laooree-modeline--flymake-determine-indicator-color (error-count warning-count note-count)
  "Determine the indicator color based on the worst diagnostic type present."
  (cond ((> error-count 0) "#3d0200")
        ((> warning-count 0) "#3d2b00")
        ((> note-count 0) "#093425")
        (t (modus-themes-get-color-value 'bg-dim))))
...
(defun laooree-modeline--flymake-diagnostics ()
  ...
  (laooree-modeline--with-indicator
   ...
   (laooree-modeline--flymake-determine-indicator-color error-count warning-count note-count)
   (modus-themes-get-color-value 'bg-mode-line-active)
   nil)))
...
(defun laooree-modeline--defining-kbd-macro ()
  ...
  (cond (defining-kbd-macro (concat (propertize "" 'face `(:background ,(modus-themes-get-color-value 'bg-mode-line-active) :foreground ,(modus-themes-get-color-value 'fg-mode-line-active)))
                                    (propertize "  Rec" 'face `(:background ,(modus-themes-get-color-value 'fg-mode-line-active) :foreground ,(modus-themes-get-color-value 'bg-mode-line-active)))
                                    (propertize " " 'face `(:background ,(modus-themes-get-color-value 'bg-mode-line-active) :foreground ,(modus-themes-get-color-value 'fg-mode-line-active)))))
        (t "")))

(defun laooree-modeline--scroll-indicator ()
  "Indicate the scrolling percentage."
  (propertize (concat (format "󰧮 %.0f" (/ (* 100.0 (point)) (point-max))) "%%") 'face 'font-lock-comment-face))
#+end_src
*** Expressions
...
(defvar-local laooree-modeline-major-mode
    '(:eval (propertize (format " %s" (laooree-modeline--major-mode-name)) 'face 'font-lock-comment-face))
...
(defvar-local laooree-modeline-scroll-indicator
    '(:eval (concat (laooree-modeline--scroll-indicator) " "))
  "Expression to indicate on the mode-line when a keyboard macro is being defined.")
...
(use-package beacon
  ...
  :init
  (setq beacon-size 20
        beacon-color (modus-themes-get-color-value 'fg-main)
